...
class RevFor {
  ...
  redoLastUndo() {
    ...
    if (last.length === 1) {
      this.insertSingle(last);
    } else this.insert(lastRedo);
...
export default class Shapes {
  ...
  draw() {
    ...
    this.figureMap.forEach((figure) => {
      ...
      ele.style.top = `${y - 25 - scrollBar.scrollPositionY / Scale.scale}px`;
  ...
  getShapesInsideFigure(figObj, key) {
    ...
    const checkIfIn = (sx, sy, swidth, sheight, obj) => {
      if (
        this.isIn(sx, sy, swidth, sheight , x, y, width, height)
      ) {
        obj.containerId = key;
